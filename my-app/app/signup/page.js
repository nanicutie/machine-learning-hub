    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .toggle-btn.inactive { background: transparent; color: rgba(196, 181, 253, 0.4); }
        .toggle-btn.inactive:hover { color: rgba(196, 181, 253, 0.65); background: rgba(168, 85, 247, 0.06); }
        .toggle-btn.active { background: linear-gradient(135deg, rgba(147, 51, 234, 0.55), rgba(124, 58, 237, 0.55)); color: #f3e8ff; box-shadow: 0 0 14px rgba(147, 51, 234, 0.3), inset 0 1px 0 rgba(255,255,255,0.08); }
        .toggle-btn.active.admin-tab { background: linear-gradient(135deg, rgba(180, 83, 9, 0.6), rgba(217, 119, 6, 0.6)); box-shadow: 0 0 14px rgba(217, 119, 6, 0.25), inset 0 1px 0 rgba(255,255,255,0.08); }

        .role-info {
          margin-bottom: 20px; padding: 12px 14px;
          border-radius: 10px; font-size: 12px; line-height: 1.5;
        }
        .role-info.user-info { background: rgba(168,85,247,0.08); border: 1px solid rgba(168,85,247,0.2); color: rgba(196,181,253,0.7); }
        .role-info.admin-info { background: rgba(217,119,6,0.1); border: 1px solid rgba(217,119,6,0.25); color: rgba(251,191,36,0.8); }
        ...
        .eyebrow { font-size: 11px; font-weight: 500; letter-spacing: 0.2em; text-transform: uppercase; color: rgba(196, 181, 253, 0.55); margin-bottom: 8px; }

        .card-title { font-family: 'Cormorant Garamond', serif; font-size: 28px; font-weight: 600; color: #f3e8ff; line-height: 1.15; margin-bottom: 6px; }

        .card-subtitle { font-family: 'Cormorant Garamond', serif; font-style: italic; font-size: 15px; color: rgba(196, 181, 253, 0.55); }

        .field-group { margin-bottom: 14px; }

        .field-label { display: block; font-size: 11px; font-weight: 500; letter-spacing: 0.12em; text-transform: uppercase; color: rgba(196, 181, 253, 0.65); margin-bottom: 7px; }
        ...
        .nav-divider { margin-top: 28px; padding-top: 22px; border-top: 1px solid rgba(168, 85, 247, 0.1); display: flex; flex-direction: column; align-items: center; gap: 10px; }
        .nav-text { font-size: 13px; color: rgba(196, 181, 253, 0.4); }
        .nav-link { font-size: 13px; font-weight: 500; color: rgba(196, 181, 253, 0.65); text-decoration: none; transition: color 0.2s; }
        .nav-link:hover { color: #d8b4fe; }
        .nav-link-subtle { font-size: 12px; color: rgba(196, 181, 253, 0.35); text-decoration: none; transition: color 0.2s; letter-spacing: 0.04em; }
      ...
      <div className="page-root">
        ...
        <div className="card">
          ...
          {/* Role Info */}
          <div className={`role-info ${role === 'admin' ? 'admin-info' : 'user-info'}`}>
            {role === 'user'
              ? '👤 User accounts can read, publish, like, and comment on articles.'
              : '🛡️ Admin accounts have full access — manage all articles and users.'}
          </div>
            ...
            <div className="icon-box">
              {role === "user" ? (
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24" stroke="rgba(216,180,254,0.9)" strokeWidth={1.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M19 7.5v3m0 0v3m0-3h3m-3 0h-3m-2.25-4.125a3.375 3.375 0 11-6.75 0 3.375 3.75 0 016.75 0zM4 19.235v-.11a6.375 6.375 0 0112.75 0v.109A12.318 12.318 0 0110.374 21c-2.331 0-4.512-.645-6.374-1.766z" />
                </svg>
              ) : (
          ...
          <div className="nav-divider">
            <p className="nav-text">Already have an account?{' '}<Link href="/login" className="nav-link">Login</Link></p>
